...
df, df_country, df_year = read_df('worldbank_climatechange.csv')
...
def balance_data(df):
  ...
  methane_emissions = remove_null_values(df[['methane_emissions']])

  greenhouse_gas_emissions = remove_null_values(df[['greenhouse_gas_emissions']])

  forest_area = remove_null_values(df[['forest_area']])

  min_length = min(len(methane_emissions), len(greenhouse_gas_emissions), len(forest_area))
  ...
  clean_data = pd.DataFrame({ 
                                ...
                                'forest_area': [forest_area[x][0] for x in range(min_length)],
                                 'methane_emissions': [methane_emissions[x][0] for x in range(min_length)]
                                 })
  return clean_data
...
scaler = MinMaxScaler()
transform_data = scaler.fit_transform(df[['greenhouse_gas_emissions', 'forest_area', 'methane_emissions']])
...
for i in range(5):
    ...
    plt.scatter(cluster_data['greenhouse_gas_emissions'], cluster_data['methane_emissions'], label=f'Cluster {i}')
...
for i in range(5):
    cluster_data = df[df['cluster'] == i]
    plt.scatter(cluster_data['forest_area'], cluster_data['methane_emissions'], label=f'Cluster {i}')
...
# create a plot showing the clusters and cluster centers using pyplot
for i in range(5):
    cluster_data = aus[aus['cluster'] == i]
    plt.scatter(cluster_data['forest_area'], cluster_data['methane_emissions'], label=f'Cluster {i}')
